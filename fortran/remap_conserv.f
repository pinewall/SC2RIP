...
!-----------------------------------------------------------------------
     ...
     &                            grid1_bound_box(3,grid1_add))
        ...
        !print 1107, min_add, max_add, num_srch_cells 
        !1107 format(i5, i5, i5)
        !***
     ...
     &        (endlat == beglat .and. endlon < beglon)) then 
          ...
          do while (beglat /= endlat .or. beglon /= endlon)
            ...
            if (lrevers) then
              print 2345, grid2_add, intrsct_lat, intrsct_lon, 
     &              beglat, beglon,
     &              endlat, endlon, begseg(1), begseg(2),"true"
            else
              print 2345, grid2_add, intrsct_lat, intrsct_lon, 
     &              beglat, beglon,
     &              endlat, endlon, begseg(1), begseg(2),"false"
            endif
 2345 format(I6,':(',F7.4,' ',F7.4,') (',F7.4,' ',F7.4,' ',
     &      F7.4,' ',F7.4,') (', F7.4,' ',F7.4, ')', A5)
     ...
     &                         grid1_center_lon(grid1_add))
            ...
            !print 1111, grid1_add, grid2_add, 
            !&          weights(1), weights(2), weights(3)
 1111 format(I6, '--', I6, F12.6,' ', F12.6, ' ',F12.6)
            !***
            ...
            if (grid2_add /= 0) then
              if (grid1_mask(grid1_add)) then
                print 1234, grid1_add, grid2_add, 
     &              weights(1), weights(2), weights(3),
     ...
     &                                  weights(num_wts+1)
            ...
            ! check weights links before storing
            !print 1234, grid1_add, grid2_add, weights(1),
            !&          weights(2), weights(3)
 1234 format(I6,'-',I6,':',F12.8,'-',F12.8,'-',F12.8,'-',
     &                     F12.8,'-',F12.8,'-',F12.8)
...
!-----------------------------------------------------------------------
     ...
     &        (endlat == beglat .and. endlon < beglon)) then 
          ...
          do while (beglat /= endlat .or. beglon /= endlon)
            ...
            if (lrevers) then
              print 2345, grid1_add, intrsct_lat, intrsct_lon, 
     &              beglat, beglon,
     &              endlat, endlon, begseg(1), begseg(2),"true"
            else
              print 2345, grid1_add, intrsct_lat, intrsct_lon, 
     ...
     &                                  weights(num_wts+1)
            ...
            !print 1234, grid2_add, grid1_add, weights(1),
            !&          weights(2), weights(3)
     ...
     &      grid2_centroid_lon(grid2_add) + weights(num_wts+3)
      ...
      open(unit=87, file="initial_weights.scrip", form="binary", 
     &          status="replace")
      write (87), wts_map1(:,1:num_links_map1)
      close(unit=87)
      ...
      do n=1,num_links_map1
        ...
        print 9999,grid1_add,grid2_add,wts_map1(1,n),
     &          wts_map1(2,n),wts_map1(3,n)
      enddo
      print *, "initial weights"

      endif
 9999 format(I6,' ',I6,'  ',F12.8,' ',F12.8,' ',F12.8)
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
     ...
     &                                norm_factor
      ...
      print *, "final_weights"
      open(unit=87, file="final_weights.scrip", form="binary", 
     &          status="replace")
      write (87), wts_map1(:,1:num_links_map1)
      close(unit=87)
...
!-----------------------------------------------------------------------
     ...
     &               beglat, beglon, endlat, endlon, begseg, lrevers)
      ...
      s1 = zero
      !print *, "init", lat1, lon1, lat2, lon2
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
     ...
     &                 srch_corner_lon(n     ,cell)
            ...
            !print *, "vec lon", vec1_lon, vec2_lon

            cross_product = vec1_lon*vec2_lat - vec2_lon*vec1_lat
            !print *, "cp 1: ", cross_product
