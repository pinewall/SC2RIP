        ...
        double grid1_lat, double grid1_lon, double grid2_lat, double grid2_lon)
{
    ...
    check_longitude(dphi, - Constant::PI, Constant::PI);
    dphi = dphi * Constant::HALF;
    ...
    f1 = Constant::HALF * (costh1*sinth1 + theta1);
    f2 = Constant::HALF * (costh2*sinth2 + theta2);

    phi1 = in_phi1 - grid1_lon;
    check_longitude(phi1, - Constant::PI, Constant::PI);

    phi2 = in_phi2 - grid1_lon;
    check_longitude(phi2, - Constant::PI, Constant::PI);

    if ((phi2-phi1) < Constant::PI && (phi2-phi1) > - Constant::PI)     // normal case
    {
        weights[2] = dphi * (phi1*f1 + phi2*f2);
    }
    else            // cross longitude bi-value line case
    {
        if (phi1 > Constant::ZERO)
            fac = Constant::PI;
        else
            fac = - Constant::PI;
        ...
        if (dphi < 0)
            fint = -fint;
        weights[2] = Constant::HALF * phi1 * (phi1-fac) * f1 -
                     Constant::HALF * phi2 * (phi2+fac) * f2 +
                     Constant::HALF * fac * (phi1+phi2) * fint;
    }
    ...
    else            // cross longitude bi-value line case
    {
        ...
        if (dphi < 0)
            fint = - fint;
        weights[num_weights+2] = Constant::HALF * phi1 * (phi1-fac) * f1 -
                                 Constant::HALF * phi2 * (phi2+fac) * f2 +
                                 Constant::HALF * fac * (phi1+phi2) * fint;
    }
}
