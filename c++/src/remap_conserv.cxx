...
void remap_conserv()
{
    ...
    memset(grid1_centroid_lat, Constant::ZERO, sizeof(double) * grid1_size);
    memset(grid1_centroid_lon, Constant::ZERO, sizeof(double) * grid1_size);
    memset(grid2_centroid_lat, Constant::ZERO, sizeof(double) * grid2_size);
    memset(grid2_centroid_lon, Constant::ZERO, sizeof(double) * grid2_size);
    ...
    // init variables
    intrsct_lat = Constant::ZERO;
    intrsct_lon = Constant::ZERO;
    begseg = new double[2];
    ...
    conserv_weights[0] = Constant::PI2;
    conserv_weights[1] = Constant::PI * Constant::PI;
    conserv_weights[2] = Constant::ZERO;
    conserv_weights[3] = Constant::PI2;
    conserv_weights[4] = Constant::PI * Constant::PI;
    conserv_weights[5] = Constant::ZERO;

    // for north pole
    for (int i = 0; i < grid1_size; i++)
    {
        if (grid1_area[i] < - THREE * Constant::PIH &&
            grid1_center_lat[i] > Constant::ZERO)
        {
            grid1_pole = i;
            break;
        }
    ...
    {
        if (grid2_area[i] < -THREE * Constant::PIH &&
            grid2_center_lat[i] > Constant::ZERO)
        {
            grid2_pole = i;
            break;
        }
    ...
    for (int n = 0; n < num_links_map; n++)
    {
        ...
        switch (norm_opt)
        {
            case NORM_OPT_DESTAREA:
                ...
                {
                    if (luse_grid2_area)
                        norm_factor = Constant::ONE / grid2_area_in[grid2_add];
                    else
                        norm_factor = Constant::ONE / grid2_area[grid2_add];
                }
                else
                    norm_factor = Constant::ZERO;
                break;
            case NORM_OPT_FRACAREA:
                if (nonzero(grid2_frac[grid2_add]))
                {
                    if (luse_grid2_area)
                        ...
                        norm_factor = Constant::ONE / grid2_frac[grid2_add];
                }
                else
                    norm_factor = Constant::ZERO;
                break;
            case NORM_OPT_NConstant::ONE:
                norm_factor = Constant::ONE;
                break;
            default:
                norm_factor = Constant::ONE;
        }
...
for (int n = 0; n < num_links_map; n++)
{
    ...
    for (int n = 0; n < grid1_size; n++)
    {
        ...
        if (grid1_centroid_lat[n] < -Constant::PIH - 0.01 ||
                grid1_centroid_lat[n] > Constant::PIH + 0.01)
            printf("Grid 1 centroid lat error @ %d\t valued %3.6f\n", n, grid1_centroid_lat[n]);
        grid1_centroid_lat[n] = Constant::ZERO;
        grid1_centroid_lon[n] = Constant::ZERO;
    }
    ...
    for (int n = 0; n < grid2_size; n++)
    {
        ...
        if (grid2_centroid_lat[n] < -Constant::PIH - 0.01 ||
                grid2_centroid_lat[n] > Constant::PIH + 0.01)
            printf("Grid 2 centroid lat error @ %d\t valued %3.6f\n", n, grid2_centroid_lat[n]);
        grid2_centroid_lat[n] = Constant::ZERO;
        grid2_centroid_lon[n] = Constant::ZERO;
    }
    // check weights
    for (int n = 0; n < num_links_map; n++)
    {
        ...
        if (norm_opt != NORM_OPT_NConstant::ONE && wts_map[num_wts * n] > 1.01)
