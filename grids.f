...
!***********************************************************************
!       Note: edited by SongShunqiang, change grid data fetching method
!           according to DongLi's netCDF format
!       Date: 2011-03-03
!***********************************************************************
      module grids
...
!-----------------------------------------------------------------------
      ...
      use parcels_io   ! Dongli's netCDF API
...
!-----------------------------------------------------------------------
     ...
     &             grid2_bound_box   ! in restricting grid searches
      ...
      !change: add grid1_corners array
      integer (kind=int_kind), dimension(:), allocatable, save ::
     &             grid1_corners_set     ! number of cell corners (not static)
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
     ...
     &  tmp_lats, tmp_lons  ! temps for computing bounding boxes
      ...
      real (kind=dbl_kind), dimension(:), allocatable, save ::
     &             density           ! density var, just for API
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
      ...
      !change
      grid1_size = 0 !fetch later using DongLi's API
...
!-----------------------------------------------------------------------
     ...
     &          grid2_bound_box (4            , grid2_size))
      ...
      !change
      call read_parcels_irregular(grid1_file, grid1_size, 
     &          grid1_corners_set,
     &          grid1_corner_lon, grid1_corner_lat, density)
      deallocate(density)   !immediately deallocate
     ...
     &          grid1_bound_box (4            , grid1_size))
      ...
      !change: just for compile; could be updated
      grid1_center_lat = sum(grid1_corner_lat,dim=1)/grid1_corners_set
      grid1_center_lon = sum(grid1_corner_lon,dim=1)/grid1_corners_set
...
!-----------------------------------------------------------------------
      ...
      !change: corner number is changing for grid1
        do n = 1, grid1_size
            grid1_corners = grid1_corners_set(n)
            !print *, "grid1_corners", grid1_corners
            grid1_bound_box(1,n) = minval(grid1_corner_lat(1:
     &          grid1_corners,n), DIM=1)
            grid1_bound_box(2,n) = maxval(grid1_corner_lat(1:
     &          grid1_corners,n), DIM=1)
            grid1_bound_box(3,n) = minval(grid1_corner_lon(1:
     &          grid1_corners,n), DIM=1)
            grid1_bound_box(4,n) = maxval(grid1_corner_lon(1:
     &          grid1_corners,n), DIM=1)
        enddo
